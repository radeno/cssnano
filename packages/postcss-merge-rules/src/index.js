...
const containsAllDeclaration = rule => rule.nodes.some(decl => decl.type === 'decl' && decl.prop === 'all');
...
function selectorMerger (browsers, compatibilityCache) {
    function partialMerge (rule, prev) {
        ...
        if (containsAllDeclaration(rule)) {
            ruleNodes = ruleNodes.slice(ruleNodes.findIndex(n => n.prop && n.prop === 'all') + 1);
        }
        let prevNodes = prev.nodes;
        if (containsAllDeclaration(prev)) {
            prevNodes = prevNodes.slice(prevNodes.findIndex(n => n.prop && n.prop === 'all') + 1);
        }
    ...
    function handleAtRule(atRule) {
        ...
        if (atRule.name !== 'media' || prev.name !== 'media') {
            return;
        }
